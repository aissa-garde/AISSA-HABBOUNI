...
function parseHistoryTimestamp(h){
  if(!h)return 0;
  if(h.generatedAtISO){
    const n=Date.parse(h.generatedAtISO);
    if(Number.isFinite(n))return n;
  }
  if(h.generatedAt){
    const raw=String(h.generatedAt).trim();
  ...
  return Number(h.createdAt||0)||0;
}
function equityEntryIsAfterReset(ts){
  const r=Number(state.equityResetAt||0);
  if(!r) return true;
  const n=typeof ts==='object' ? parseHistoryTimestamp(ts) : Number(ts||0);
  return n>0 && n>=r;
}

function historyIsValidForRecovery(h,key){
  if(!h) return false;
  // Une entrée compte uniquement si elle a été explicitement validée.
  return !!(h.confirmed===true || state.confirmedPlanningKeys?.[key]===true) && h.includeInEquity!==false;
}
function recoveryHistoryEntries(){
  const entries=[];
  const seen=new Set();
  Object.entries(state.planningHistory||{}).forEach(([key,raw])=>{
    const h=raw?.planning ? raw : {planning:raw,fixed:[],absences:[],period:null};
    if(!historyIsValidForRecovery(h,key)) return;
    const period=h.period||{};
    const start=period.start?dateFromISO(period.start):null;
    const end=period.end?dateFromISO(period.end):null;
    // 1. J et N enregistrés dans le planning validé.
    Object.values(h.planning||{}).flat().forEach(x=>{
      if(!x?.doctor||!x?.date||!["J","N"].includes(x.type)) return;
      const sig=`${x.doctor}|${x.date}|${x.type}`;
      if(seen.has(sig)) return;
      seen.add(sig);
      entries.push({doctor:x.doctor,date:x.date,type:x.type,source:key});
    });
    // 2. G/F et éventuels J/N fixes du snapshot du même planning validé.
    (h.fixed||[]).forEach(x=>{
      if(!x?.doctor||!x?.date||!["J","N","G","F"].includes(x.type)) return;
      const d=dateFromISO(x.date);
      if(start&&end&&d&&(d<start||d>end)) return;
      const sig=`${x.doctor}|${x.date}|${x.type}`;
      if(seen.has(sig)) return;
      seen.add(sig);
      entries.push({doctor:x.doctor,date:x.date,type:x.type,source:key});
...
function calculateRecovery(){
  const result={};
  state.doctors.forEach(d=>result[d.name]={J:0,N:0,G:0,F:0,total:0,recupAcquise:0,recupPrise:0,recupSolde:0});

  // AFFICHER SCORE = reconstruction complète depuis TOUS les plannings validés.
  recoveryHistoryEntries().forEach(x=>{
    if(!result[x.doctor]) return;
    result[x.doctor][x.type]++;
    result[x.doctor].total++;
    if(["N","G","F"].includes(x.type)) result[x.doctor].recupAcquise+=2;
  });

  // Les récupérations prises sont elles aussi relues dans les snapshots des
  // plannings validés, afin de ne jamais dépendre d'un compteur temporaire.
  const takenSeen=new Set();
  Object.entries(state.planningHistory||{}).forEach(([key,raw])=>{
    const h=raw?.planning ? raw : {planning:raw,absences:[]};
    if(!historyIsValidForRecovery(h,key)) return;
    const period=h.period||{};
    const pStart=period.start?dateFromISO(period.start):null;
    const pEnd=period.end?dateFromISO(period.end):null;
    (h.absences||[]).forEach(a=>{
      if(!a?.doctor || !["RÉCUP","RECUP"].includes(String(a.type||'').toUpperCase()) || !result[a.doctor]) return;
      let d=dateFromISO(a.start), e=dateFromISO(a.end);
      if(!d||!e||e<d) return;
      if(pStart&&d<pStart) d=new Date(pStart);
      if(pEnd&&e>pEnd) e=new Date(pEnd);
      while(d<=e){
        const sig=`${a.doctor}|${iso(d)}`;
        if(!takenSeen.has(sig)){
          takenSeen.add(sig);
          result[a.doctor].recupPrise++;
        }
        d.setDate(d.getDate()+1);
      }
    });
  });
  Object.values(result).forEach(x=>x.recupSolde=x.recupAcquise-x.recupPrise);
  return result;
}
function rebuildRecoveryLedger(){
  const c=calculateRecovery();
  state.recoveryLedger={};
  state.doctors.forEach(d=>{
    const x=c[d.name]||{recupAcquise:0,recupPrise:0,recupSolde:0};
    state.recoveryLedger[d.name]={
      solde:Number(x.recupSolde||0),
      totalAcquise:Number(x.recupAcquise||0),
      totalPrise:Number(x.recupPrise||0),
...
function weeklyRepeatPenalty(doc,date,type,planning){
  ...
  // dans la même semaine ouvrable (lundi→vendredi). Ce n'est JAMAIS
  // une contrainte bloquante : la couverture J+N et les contraintes du
  // médecin restent prioritaires.
  if(!doc || !date || !["J","N"].includes(type)) return 0;
  ...
  let count=0;
  for(const x of Object.values(planning||{}).flat()){
    if(x?.doctor!==doc.name || x.type!==type || !x.date) continue;
    const xd=dateFromISO(x.date);
    if(xd>=monday && xd<=friday) count++;
  }
  // Les J/N fixes saisis manuellement font également partie de la semaine.
...
function candidateScore(doc,type,c,monthly,date,planning){
  ...
  const weeklyRepeat=weeklyRepeatPenalty(doc,date,type,planning);
  ...
  // La répétition J/J ou N/N est une préférence, pas une interdiction.
  // Elle n'écrase donc jamais la charge pondérée ni la charge mensuelle.
  return total*100000 + month*1000 + weeklyRepeat*100 + shiftBalance*20 + imbalance*5;
}
...
function generate(){
  const buttons=[...document.querySelectorAll('button[onclick="generate()"]')];
  buttons.forEach(b=>{b.disabled=true; b.dataset.generateOriginal=b.innerHTML;});
  const msg=document.getElementById("genMessage");
  if(msg) msg.innerHTML='<div class="gen-wait"><span class="gen-spinner"></span><span>Please wait… génération du planning en cours.</span></div>';
  // Laisser le navigateur afficher l'indicateur avant le calcul, qui peut être long.
  setTimeout(()=>{
    try{ generateCore(); }
    catch(err){
      console.error(err);
      if(msg) msg.innerHTML='<div class="warn"><b>Erreur pendant la génération.</b><br>Veuillez réessayer.</div>';
    } finally {
      buttons.forEach(b=>{b.disabled=false; if(b.dataset.generateOriginal)b.innerHTML=b.dataset.generateOriginal;});
    }
  },50);
}
function generateCore(){
  ...
  function randomPick(candidates,st,type,date){
    ...
    // proche), l'ordre de la liste des médecins ne donne aucun avantage
    // systématique aux premiers médecins.
    const randomized=candidates.slice();
    for(let i=randomized.length-1;i>0;i--){
      const j=Math.floor(Math.random()*(i+1));
      [randomized[i],randomized[j]]=[randomized[j],randomized[i]];
    }
    const ranked=randomized.map(doc=>{
      ...
      const score=candidateScore(doc,type,st.counts,st.monthly,date,st.planning);
  ...
  function expandRandom(initialState){
    ...
    for(const date of dayOrder){
      ...
      for(const type of missingTypes){
        ...
        const chosen=randomPick(candidates,st,type,date);
  ...
  function finalMetric(st){
    ...
    // Préférence d'équilibrage : pénaliser les 2e J et 2e N d'un même
    // médecin dans une même semaine ouvrable. Cette pénalité reste très
    // inférieure aux contraintes absolues et à l'équité de charge.
    let weeklyRepeats=0;
    const weekBuckets={};
    for(const x of Object.values(st.planning||{}).flat()){
      if(!x?.doctor || !x.date || !["J","N"].includes(x.type)) continue;
      const dt=dateFromISO(x.date), wd=dt.getDay();
      if(wd===0 || wd===6) continue;
      const monday=new Date(dt); monday.setDate(dt.getDate()-(wd-1)); monday.setHours(0,0,0,0);
      const key=`${x.doctor}|${iso(monday)}|${x.type}`;
    ...
    Object.values(weekBuckets).forEach(n=>{if(n>1) weeklyRepeats+=n-1;});
    let metric=missing*1e15 + restCount*1e14 + weeklyRepeats*100;
...
function confirmCurrentPlanning(){
  ...
  const per=getPeriodDates();
  if(!state.planning || !Object.values(state.planning).flat().length){return alert("Aucun planning à confirmer.");}
  const old=state.planningHistory?.[per.key];
  if(old?.confirmed){return alert("Ce planning est déjà validé.");}
  if(!confirm("Valider ce planning ?"))return;

  const now=Date.now();
  state.planningHistory[per.key]={
    planning:JSON.parse(JSON.stringify(state.planning)),
    fixed:JSON.parse(JSON.stringify(state.fixed||[])),
    absences:JSON.parse(JSON.stringify(state.absences||[])),
    period:{start:iso(per.start),end:iso(per.end),type:per.type,weeks:per.weeks||1},
    confirmed:true,
    confirmedAtISO:now,
    generatedAt:old?.generatedAt||new Date(now).toLocaleString('fr-FR',{dateStyle:'short',timeStyle:'short'}),
    generatedAtISO:old?.generatedAtISO||now,
    includeInEquity:true
  };
  state.confirmedPlanningKeys[per.key]=true;
  // Le score est toujours reconstruit depuis les validations : aucune valeur
  // temporaire n'est ajoutée manuellement au compteur.
  rebuildRecoveryLedger();
  awaitSaveAndRender();
}
async function awaitSaveAndRender(){
  await save();
  const msg=document.getElementById('genMessage');
  if(msg)msg.innerHTML='<div class="ok"><b>Planning validé.</b></div>';
  renderAll();
}
function recalculateRecoveryScores(){
  if(!isAdmin())return alert("Le recalcul des scores est réservé à l'administrateur.");
  // Bouton AFFICHER SCORE : on relit immédiatement tous les plannings validés.
  state.recoveryDisplayReset=false;
  state.equityResetAt=0;
  rebuildRecoveryLedger();
  save();
  renderStats();
  const msg=document.getElementById('genMessage');
  if(msg) msg.innerHTML='<div class="ok"><b>Scores de récupération recalculés à partir des plannings validés.</b></div>';
}
function resetAllEquityCounters(){
  if(!isAdmin())return alert("Cette action est réservée à l'administrateur.");
  if(!confirm("Remettre à 0 l'affichage des compteurs J, N, G et F ainsi que les scores de récupération ?"))return;
  // RESET met seulement l'affichage à zéro. Les plannings validés sont conservés
  // et seront relus intégralement au prochain clic sur AFFICHER SCORE.
  state.recoveryDisplayReset=true;
  state.recoveryLedger={};
  state.equityResetAt=0;
...
function renderStats(){
  ...
  const c=state.recoveryDisplayReset
    ? Object.fromEntries(state.doctors.map(d=>[d.name,{J:0,N:0,G:0,F:0,total:0,recupAcquise:0,recupPrise:0,recupSolde:0}]))
    : calculateRecovery();
